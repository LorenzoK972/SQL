...
SELECT MEAL_NAME, COUNT(MEAL_NAME) AS TIMES_MEAL_ORDERED 
FROM MEAL_ORDER_WITH_DETAILS GROUP BY MEAL_NAME;
SELECT MEAL_NAME, COUNT(MEAL_NAME), FORMAT(SUM(PRICE), 2) AS PRICE_MEAL 
FROM MEAL_ORDER_WITH_DETAILS WHERE PRICE > 15.00 GROUP BY MEAL_NAME;
SELECT MEAL_NAME, CALORIES, COUNT(MEAL_NAME) AS TIMES_MEAL_ORDERED 
FROM MEAL_ORDER_WITH_DETAILS WHERE CALORIES > 500 GROUP BY MEAL_NAME, CALORIES;
SELECT FORMAT(SUM(PRICE), 2) AS TOTAL_PRICE_MEAL_WITH_S_IN_THE_NAME 
FROM MEAL_ORDER_WITH_DETAILS WHERE LOWER(MEAL_NAME) LIKE '%s%' GROUP BY MEAL_NAME;
SELECT MEAL_NAME, COUNT(*) AS OREDERS_BEFORE_2023_05_21_18_00_00
FROM MEAL_ORDER_WITH_DETAILS WHERE TIME_MEAL_ORDERED < '2023-05-21 18:00:00' GROUP BY MEAL_NAME;