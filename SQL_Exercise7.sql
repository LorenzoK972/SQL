...
SELECT MAX(PRICE) AS MAX_PRICE FROM MEAL_ORDER_WITH_DETAILS WHERE CALORIES > 500;
SELECT AVG(CALORIES) AS AVG_CALORIES FROM MEAL WHERE PRICE > 15;
SELECT NAME, PRICE AS MAX_MEAL_PRICE FROM MEAL HAVING PRICE = (SELECT MAX(PRICE) FROM MEAL);
SELECT MEAL_NAME, PRICE AS UNDER_AVG_MEAL_PRICE1 FROM MEAL_ORDER_WITH_DETAILS
GROUP BY MEAL_NAME, PRICE HAVING PRICE < (SELECT AVG(PRICE) FROM MEAL_ORDER_WITH_DETAILS);
SELECT MEAL_NAME, PRICE, CALORIES AS UNDER_AVG_MEAL_PRICE2 FROM MEAL_ORDER_WITH_DETAILS
GROUP BY MEAL_NAME, PRICE, CALORIES HAVING PRICE < (SELECT AVG(PRICE) FROM MEAL_ORDER_WITH_DETAILS) AND CALORIES > 600;